...
Site.FormManager = function() {
    ...
    this.init = function() {
		...
		this.action = $('#request-form input[name="action"]').val();
        ...
        inputs.push(nameInput, companyInput, emailInput, needsInput, phoneInput);
        ...
        for (var i = 0; i < inputBlocks.length; i++) {
            ...
            input.addEventListener('blur', function(ev) {
                ev.stopPropagation();
                ev.preventDefault();
                if((ev.srcElement.value == "" || ev.srcElement.value == null) && ev.srcElement.id !== "PHONE") {
                    // it's empty
    ...
    this.showForm = function(section) {
        if (section == 5) {
            ...
            self.requestButton.span.innerHTML = self.lang == "RU" ? "Отправить" : "Submit";
        } else {
            form.classList.remove("active");
            self.requestButton.classList.remove("submit");
            self.requestButton.span.innerHTML = self.lang == "RU" ? "Узнать больше" : "Request a Quote"
        }
    };
    ...
    this.requestFormSubmit = function() {
        ...
        if (phoneInput.value.replace(/[^0-9.]/g, '').length == 11 && nameInput.value != "" && nameInput.value != null && companyInput.value != "" && companyInput.value != null && String(emailInput.value) != "" && String(emailInput.value) != null && needsInput.value != "" && needsInput.value != null) {
            requestForm.action = submitURL;
			
			$.ajax({
				url: "../../engine/module/ajax/landing.php",
				type: "POST",
				data:  $('#request-form').serialize().replace(/\+/g,'%20'),
				success: function (text) {
						if(text.status == 'ok'){
								console.log(text);
						}else{
							if(typeof text.msg != "undefined")
								$('#success-message').text(text.msg);
						}
				}
			});
    ...
    this.errorCheck = function() {
        for (i=0; i<inputs.length; i++) {
			...
			if(inputs[i].id == "PHONE"){
				if(inputs[i].value.replace(/[^0-9.]/g, '').length !== 11)
					inputs[i].parentElement.classList.add("error");
			}
